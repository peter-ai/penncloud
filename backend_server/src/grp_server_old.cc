...
int KVSClient::send_operation_to_secondaries(std::vector<char> inputs)
{
    ...
    // ! this connection now needs to wait until it gets a response from all of the connections it opened

    // ! once it has received the messages, it can figure out if this was the operation that it was supposed to perform next
